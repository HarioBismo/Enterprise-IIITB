...
public class iStudent extends dataSource{
	...
	public String getStudent() throws SQLException, ParseException
	{
        ...
        for (int i = 0; i < stud.size(); i++) {
        	...
        	table= table+"<tr id='update"+stud.get(i).getRollNo()+"'><td>" + stud.get(i).getRollNo() + "</td>" + 
        			...
        			"<td><img src='picture/" +stud.get(i).getPhoto() + "' height='42' width='42' /></td>" +
        			"<td><button type='button' class='btn btn-default' id='editStudent' onclick='editStudent("+ stud.get(i).getRollNo()+ ")' >Edit</button></td>" +
        			"<td><button type='button' class='btn btn-default' id='delStudent' onclick='delStudent("+ stud.get(i).getRollNo()+ ")' >Delete</button></td></tr>" ;
        }
        table=table+"</table>";
	...
	@Path("/delete")
	@Consumes(MediaType.APPLICATION_JSON)
	public void DeleteStudent(String rool_No) throws SQLException, ParseException
	{
		new dataSource().delete(rool_No);
	}
